...
function LSTM.lstm(input_size, output_size, rnn_size, n, dropout) -- 512, 9567+1, 512, 1, 0.5
  ...
  for L = 1,n do -- 1
    table.insert(inputs, nn.Identity()()) -- prev_c[L], c at time t-1 
    table.insert(inputs, nn.Identity()()) -- prev_h[L], h at time t-1
  end
  ...
  for L = 1,n do
    ...
    local prev_c = inputs[L*2] -- prev_c = inputs[2]
    local prev_h = inputs[L*2+1] -- prev_h = inputs[3]
    -- the input to this layer
    if L == 1 then 
      x = inputs[1] -- network input
